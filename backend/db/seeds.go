...
// seedTracker seeds tracker categories and tasks if empty
func seedTracker(tx *sql.Tx) error {
	var count int
	err := tx.QueryRow("SELECT COUNT(*) FROM tracker_categories").Scan(&count)
	if err != nil {
		return err
	}
	if count > 0 {
	...
	log.Println("[Seeds] Seeding initial tracker data...")

	var categories []SeedCategory
	if err := json.Unmarshal([]byte(initialTrackerDataJSON), &categories); err != nil {
		return err
	}
	...
	for _, cat := range categories {
		...
		for _, task := range cat.Tasks {
			dueDate := AddDays(task.StartDate, task.Duration)

			_, err := tx.Exec(
				`INSERT INTO tracker_tasks (id, category_id, row_id, title, owner, start_date, duration, progress, status, due_date) 
				 VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)`,
				task.ID, cat.ID, task.RowID, task.Title, task.Owner, task.StartDate, task.Duration, task.Progress, task.Status, dueDate,
			)
			if err != nil {
				return err
			}
	...
	log.Println("[Seeds] Tracker data seeded successfully!")
	return nil
}
